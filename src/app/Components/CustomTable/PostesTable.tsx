  ...
  const handleDelete = async () => {
    try {
      if (deleteNameInput === posteToDelete?.name) {
        const response = await deleteWithAuth(`api/delete-postes/${posteToDelete.id}/`);
    ...
    {ligneOptions.map((ligne) => (
      <label key={ligne.id} className="flex items-center space-x-2">
        <input
          ...
          onChange={(event) => handleEditInputChange(event, 'lignes', ligne.id)}
          className="mr-2"
        />
